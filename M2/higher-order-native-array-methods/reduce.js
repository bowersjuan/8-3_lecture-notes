...
function stringConcat(arr) {
  let concatinatedString = arr.reduce((currentElement, nextElement) => {
    return currentElement.toString() + nextElement.toString();
  });
  return concatinatedString;
}

console.log("stringConcat", stringConcat([1, 2, 3])); // "123"
/**
 * 5
 *
...
function totalVotes(arr) {
  let votes = arr.reduce((acc, nextVote) => {
    if (nextVote.voted) {
      return ++acc;
    } else {
      return acc;
...
function shoppingSpree(arr) {
  let totalCost = arr.reduce((acc, nextItem) => {
    return acc + nextItem.price;
  }, 0);
...
console.log("shoppingSpree", shoppingSpree(wishlist)); // 227005
/**
 * 7
 *
...
function flatten(arr) {
  let flattenedArr = arr.reduce((acc, nextArray) => {
...
console.log("flatten:", flatten(arrays)); // ["1", "2", "3", true, 4, 5, 6];
